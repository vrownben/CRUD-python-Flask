...
# Creando una funcion para obtener la lista de coches.
def listaCoches():
    conexion_MySQLdb = connectionBD()  # creando mi instancia a la conexion de BD
    cur = conexion_MySQLdb.cursor(dictionary=True)

    querySQL = "SELECT * FROM coches ORDER BY id"
...
def updateCarro(id=''):
    ...
    cursor.execute("SELECT * FROM coches WHERE id = %s LIMIT 1", [id])
    resultQueryData = cursor.fetchone()  # Devolviendo solo 1 registro
    return resultQueryData


def registrarCarro(marca='', modelo='', year='', color='', puertas='', nuevoNombreFile=''):
    conexion_MySQLdb = connectionBD()
    cursor = conexion_MySQLdb.cursor(dictionary=True)

    sql = ("INSERT INTO coches(marca, modelo, year, color, puertas, foto) VALUES (%s, %s, %s, %s, %s, %s)")
    valores = (marca, modelo, year, color, puertas, nuevoNombreFile)
...
def detallesdelCarro(idCarro):
    conexion_MySQLdb = connectionBD()
    cursor = conexion_MySQLdb.cursor(dictionary=True)
    cursor.execute("SELECT * FROM coches WHERE id ='%s'" % (idCarro,))
...
def recibeActualizarCarro(marca, modelo, year, color, puertas, nuevoNombreFile, idCarro):
    conexion_MySQLdb = connectionBD()
    cur = conexion_MySQLdb.cursor(dictionary=True)
    cur.execute("""
            UPDATE coches
            SET 
                ...
                foto    = %s
            WHERE id=%s
            """, (marca, modelo, year, color, puertas, nuevoNombreFile,  idCarro))
