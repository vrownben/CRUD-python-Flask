...
#Creando mi decorador para el home, el cual retornara la Lista de Coches
@app.route('/', methods=['GET','POST'])
def inicio():
    return render_template('public/layout.html', miData = listaCoches())
...
def formAddCarro():
    if request.method == 'POST':
        ...
        if(request.files['foto'] !=''):
            ...
            resultData = registrarCarro(marca, modelo, year, color, puertas, nuevoNombreFile)
            if(resultData ==1):
                return render_template('public/layout.html', miData = listaCoches(), msg='El Registro fue un éxito', tipo=1)
            else:
...
def formViewUpdate(id):
    if request.method == 'GET':
        ...
        if resultData:
            ...
            return render_template('public/layout.html', miData = listaCoches(), msg='No existe el carro', tipo= 1)
    else:
        return render_template('public/layout.html', miData = listaCoches(), msg = 'Metodo HTTP incorrecto', tipo=1)          
...
def  formActualizarCarro(idCarro):
    if request.method == 'POST':
        ...
        if(request.files['foto']):
            ...
            resultData = recibeActualizarCarro(marca, modelo, year, color, puertas, fotoForm, idCarro)
        else:
            fotoCarro  ='sin_foto.jpg'
            """ resultData = recibeActualizarCarro(marca, modelo, year, color, puertas, favorito, fotoCarro, idCarro) """
            resultData = recibeActualizarCarro(marca, modelo, year, color, puertas, fotoCarro, idCarro)

        if(resultData ==1):
            return render_template('public/layout.html', miData = listaCoches(), msg='Datos del carro actualizados', tipo=1)
        else:
            msg ='No se actualizo el registro'
            return render_template('public/layout.html', miData = listaCoches(), msg='No se pudo actualizar', tipo=1)
